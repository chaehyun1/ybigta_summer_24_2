...
class ExperienceBuffer:
    ...
    def sample(self) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        '''TODO'''
        s, a, r, s_bar, done = map(np.array, zip(*self.buffer))
        self.buffer.clear()
        return torch.FloatTensor(s), torch.FloatTensor(a), torch.FloatTensor(r).unsqueeze(1), torch.FloatTensor(s_bar), torch.FloatTensor(done).unsqueeze(1)
    ...
    def update(self) -> None:
        ...
        for _ in range(self.n_epochs):
            for batch in loader:
                ...
                surr1 = adv_batch * ratio
                surr2 = adv_batch * torch.clamp(ratio, 1 - self.clip_ratio, 1 + self.clip_ratio)
